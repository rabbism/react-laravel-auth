...
const Signup=()=>{
   ...
   async function  signup()
       {
        await axios.post("https://api.salesolutionbd.com/api/v1/register",user)
        .then(function (response) {
           setErrors({
            //    successMsg:response.data.susccess
           });
         })  		
         .catch(function (err) {
    ...
    return(
        <Grid>
            <Paper elevation={10} style={paperStyle}>
                ...
                <TextField label='Confirm Password'  name="c_password" value={c_password}  onChange={e => onInputChange(e)} placeholder='Confirm password' type='password' fullWidth required/>
